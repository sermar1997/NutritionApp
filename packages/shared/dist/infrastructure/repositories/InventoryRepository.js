...
import { InventoryItemStatus, StorageLocation, InventoryChangeType } from '../../core/models/inventory';
/**
...
export class InventoryRepository extends BaseRepository {
    ...
    constructor(storageService) {
        ...
        this.logStorageKey = 'inventory_logs';
        this.shoppingListStorageKey = 'shopping_list';
    }
    /**
    ...
    async logInventoryChange(log) {
        try {
            const logs = await this.storageService.get(this.logStorageKey) || [];
            logs.push(log);
     ...
     */
    async getChangeLogs(itemId, limit) {
        try {
            const logs = await this.storageService.get(this.logStorageKey) || [];
    ...
    async getShoppingList() {
        try {
            return await this.storageService.get(this.shoppingListStorageKey) || [];
        }
        catch (error) {
     ...
     * @returns True if successfully updated, false otherwise
     */
    async markAsPurchased(id, isPurchased) {
     ...
     */
    async addInventoryFromPurchased(shoppingListIds) {
        try {
            if (!shoppingListIds.length) {
                return false;
            }
            const shoppingList = await this.getShoppingList();
            const purchasedItems = shoppingList.filter(item => shoppingListIds.includes(item.id) && item.isPurchased);
            ...
            const updatedShoppingList = shoppingList.filter(item => !shoppingListIds.includes(item.id));
