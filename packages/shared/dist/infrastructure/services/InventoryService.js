...
import { InventoryAlertType, InventoryItemStatus } from '../../core/models/inventory';
...
export class InventoryService {
     ...
     */
    constructor(inventoryRepository, recipeRepository) {
     ...
     */
    async generateShoppingList(includeOutOfStock, includeLowStock) {
        try {
            ...
            const now = new Date();
            const shoppingList = filteredItems.map(item => ({
                id: uuidv4(),
                name: item.name,
        ...
        try {
            ...
            for (const item of items) {
                ...
                if (newStatus !== InventoryItemStatus.EXPIRED) {
                    ...
                    else if (item.lowStockThreshold &&
     ...
     */
    async useItemsForRecipe(recipeId, servings) {
        try {
            ...
            for (const recipeIngredient of recipe.ingredients) {
                ...
                const inventoryItem = inventoryItems.find(item => item.id === recipeIngredient.ingredientId);
                if (!inventoryItem)
                    continue;
     ...
     */
    async sortItems(sortBy, ascending) {
        try {
            const items = await this.inventoryRepository.getAll();
