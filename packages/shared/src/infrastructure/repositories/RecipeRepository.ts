...
export class RecipeRepository 
  ...
  private readonly favoritesStorageKey = 'favorites_recipes';
   ...
   */
  async add(recipeData: RecipeCreateDto): Promise<Recipe> {
    try {
      const now = new Date();
   ...
   */
  async update(id: string, updates: RecipeUpdateDto): Promise<boolean> {
    try {
      const recipes = await this.getAll();
  ...
  private async getFavoriteIds(): Promise<string[]> {
    try {
      const favorites = await this.storageService.get<string[]>(this.favoritesStorageKey);
      return favorites || [];
    } catch (error) {
