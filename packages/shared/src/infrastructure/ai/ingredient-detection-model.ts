...
import { Ingredient, IngredientCategory, DetectedIngredient, UnitOfMeasurement } from '../../core/models/ingredient';
 ...
 */
export class IngredientDetectionModel implements ModelHandler<IngredientDetectionInput, InfraImageAnalysisResult> {
   ...
   */
  public async process(input: IngredientDetectionInput): Promise<InfraImageAnalysisResult> {
    try {
   ...
   */
  private processPredictions(predictions: tf.Tensor): DetectedIngredient[] {
      ...
      try {
        ...
        if (Array.isArray(predictions) && predictions.length >= 4) {
          ...
          if ('boxes' in predictions && 'scores' in predictions && 
            ...
            return [
              (predictions as any).boxes,
              (predictions as any).scores,
              (predictions as any).classes,
              (predictions as any).valid_detections
            ];
          } else {
            ...
            if (boxesKey && scoresKey && classesKey && detectionKey) {
              return [
                (predictions as any)[boxesKey],
                (predictions as any)[scoresKey],
                (predictions as any)[classesKey],
                (predictions as any)[detectionKey]
              ];
            }
          }
        }
 ...
 */
export function createIngredientDetectionModel(): ModelHandler<IngredientDetectionInput, InfraImageAnalysisResult> {
